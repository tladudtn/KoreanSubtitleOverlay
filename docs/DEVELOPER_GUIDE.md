...
│   ├── KoreanSubtitleOverlay.ini  배포용 설정 템플릿 (아래 "설정 파일" 참고)
│   ├── fonts\                커스텀 FontPath용 폰트 파일을 넣는 자리 (fonts\README.md)
...
`FontSize`/`OffsetX`/`OffsetY`/`Debug`/`FontPath` 다섯 값은 하드코딩
상수가 아니라 `.asi`와 같은 폴더의 `KoreanSubtitleOverlay.ini`에서 매
실행 시 한 번 읽어온다(`src/dllmain.cpp`의 `Config` 구조체,
`LoadConfig()`). 저장소 루트의 `KoreanSubtitleOverlay.ini`가 기본값이
채워진 배포용 템플릿이고, `fonts/` 폴더(`fonts/README.md` 참고)가
`FontPath`로 가리킬 커스텀 폰트 파일을 넣는 자리다.
...
끝나 있어야 `FontSize`/`FontPath`가 폰트 로드에 반영된다.

**ini 파일과 폰트 경로를 찾는 방법**: 게임의 현재 작업 디렉터리에
의존하지 않고, `GetModuleFileNameA(g_hModule, ...)`로 **이 DLL 자신의
실제 경로**를 얻어서 그 폴더를 기준으로 삼는다(`GetModuleDir()`, 최초
호출 시 한 번만 계산해서 캐시). `BuildIniPath()`는 이 폴더 +
`KoreanSubtitleOverlay.ini`. `ResolveFontPath()`도 같은 폴더를 기준으로
`FontPath`의 상대 경로(예: `fonts/MyFont.ttf`)를 절대 경로로 바꾸는데,
`IsAbsolutePath()`로 드라이브 문자(`C:...`)나 UNC 경로(`\\서버\공유`)는
이미 절대 경로이므로 그대로 통과시킨다. `g_hModule`은 `DllMain`의
`DLL_PROCESS_ATTACH`에서 넘어오는 `hModule`을 저장해둔 것.
...
`ReadFloatSetting`/`ReadBoolSetting`/`ReadStringSetting`).
`GetPrivateProfileIntA`를 안 쓰는 이유: 음수 오프셋(예: `OffsetY=-20`)을
안정적으로 파싱하려고 문자열 경로를 택함. ini 파일 자체가 없거나 키가
빠져 있으면 각 함수가 `Config` 구조체의 기본값(코드 상단, 이 옵션이
생기기 전의 원래 하드코딩 값과 동일 - `FontPath`는 빈 문자열)으로
폴백하므로, ini 없이 `.asi`만 깔아도 이전과 동일하게 동작한다.

**`FontPath`가 지정됐을 때의 로드 순서**(`LoadKoreanFont()`): `FontPath`가
비어있지 않으면 먼저 그 파일로 `AddFontFromFileTTF`를 시도하고,
성공하면 그걸로 끝(시스템 폰트는 아예 안 건드림). 그 파일이 없거나
로드에 실패하면 `[warn]` 로그를 남기고 기존의 `malgunbd.ttf` →
`malgun.ttf` → ImGui 기본 폰트 폴백 체인으로 그대로 넘어간다 - 즉
`FontPath`가 깨져 있어도 오버레이 자체가 안 뜨는 일은 없다.
...
| `[config] path=... FontSize=... OffsetX=... OffsetY=... Debug=... FontPath=...` | `LoadConfig()`, 초기화 초반 1회 | ini 파일을 실제로 어느 경로에서 읽었는지, 각 값이 뭘로 최종 확정됐는지 - 설정이 안 먹히는 것 같을 때 가장 먼저 볼 것 | 항상 |
| `[init] loaded custom FontPath: ...` / `[warn] FontPath "..." failed to load, ...` | `LoadKoreanFont()`, 초기화 중 1회 | 커스텀 `FontPath`가 실제로 로드됐는지, 실패해서 시스템 맑은 고딕으로 폴백했는지 | 항상 |
