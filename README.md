...
- `KoreanSubtitleOverlay.ini`(.asi와 같은 폴더)로 글자 크기, 위치 오프셋
  (X/Y), 커스텀 폰트 경로, 디버그 로그 on/off를 설정 가능 - 아래 "설정" 항목 참고

## 설정

`.asi`와 같은 폴더의 `KoreanSubtitleOverlay.ini`에서 다음 값을 바꿀 수
있습니다 (저장소 루트의 `KoreanSubtitleOverlay.ini`가 기본값 템플릿).
게임을 완전히 재시작해야 반영됩니다.

```ini
[Subtitle]
FontSize=39   ; 자막 글자 크기(픽셀)
OffsetX=0     ; 자막 위치를 화면 중앙 기준 오른쪽(+)/왼쪽(-)으로 이동(픽셀)
OffsetY=0     ; 자막 위치를 아래(+)/위(-)로 이동(픽셀)
Debug=no      ; yes로 켜면 로그에 매 줄/매 프레임 단위 상세 진단 정보 추가
FontPath=     ; 비워두면 시스템 맑은 고딕 사용. fonts/ 폴더에 폰트를 넣고
              ; FontPath=fonts/MyFont.ttf 처럼 상대 경로로 지정 가능
```

`fonts/` 폴더(이 저장소 루트, `.asi`와 같이 배포)에 원하는 `.ttf`/`.otf`를
넣고 `FontPath`로 가리키면 시스템 폰트 대신 그 폰트를 씁니다 - 지정한
파일을 못 찾으면 자동으로 시스템 맑은 고딕으로 되돌아갑니다. 자세한
사용법은 [fonts/README.md](fonts/README.md) 참고.

ini 파일이 없거나 특정 키가 빠져 있으면 위 기본값(이 설정 기능이 생기기
전의 원래 동작)으로 폴백합니다.
...
2. GTA SA 설치 폴더의
   ...
   위치로 복사합니다. 글자 크기/위치/폰트를 조정하고 싶다면
   `KoreanSubtitleOverlay.ini`(커스텀 폰트를 쓸 경우 `fonts/` 폴더도)를
   같은 폴더에 같이 복사하세요(둘 다 선택사항 - 없어도 기본값으로
   동작합니다. 자세한 항목은 위 "설정" 참고).
